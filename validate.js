...
function autoSaveForm() {
    ...
    localStorage.setItem('validationFormDraft', JSON.stringify({
        data,
...
function loadFormDraft() {
    ...
    try {
        ...
        const draftAge = (Date.now() - new Date(timestamp).getTime()) / (1000 * 60 * 60); // hours
...
function validateValidationForm() {
    ...
    if (problemStatement && problemStatement.value.trim().length < 50) {
        addValidationFeedback(problemStatement, false, 'Problem statement should be at least 50 characters');
        isValid = false;
    }
    ...
    if (solutionDescription && solutionDescription.value.trim().length < 50) {
        addValidationFeedback(solutionDescription, false, 'Solution description should be at least 50 characters');
        isValid = false;
    }
...
function generateValidationResults(data) {
    ...
    return {
        ...
        analysis: {
            problem_quality: Math.round((problemLength / 20 + problemComplexity * 5) * 5),
            solution_quality: Math.round((solutionLength / 20 + solutionComplexity * 5) * 5),
            market_potential: Math.round(baseScore * marketSizeMultiplier[data.marketSize] * 0.3),
...
function analyzeTextComplexity(text) {
    ...
    if (text.includes('innovative') || text.includes('unique') || text.includes('revolutionary')) complexity += 0.5;
    ...
    return [
        {
            ...
            score: Math.max(20, Math.min(100, overallScore + (Math.random() - 0.5) * variance))
        },
        {
...
function generateRecommendations(scores, data) {
    ...
    const problemSolutionScore = scores.find(s => s.category === 'Problem-Solution Fit')?.score || 0;
    if (problemSolutionScore < 70) {
        recommendations.push({
            category: 'Problem-Solution Fit',
            recommendation: 'Conduct deeper customer interviews to validate that your solution truly addresses the core problem. Consider creating user personas and journey maps.'
        });
    }
    
    // Market Opportunity recommendations
    const marketScore = scores.find(s => s.category === 'Market Opportunity')?.score || 0;
    if (marketScore < 70) {
        recommendations.push({
            category: 'Market Research',
            recommendation: 'Expand your market research to include competitor analysis, market size validation, and customer willingness to pay studies.'
        });
    }
    
    // Business Model recommendations
    const businessScore = scores.find(s => s.category === 'Business Model')?.score || 0;
    if (businessScore < 70) {
        recommendations.push({
            category: 'Business Model',
            recommendation: 'Test different pricing models and revenue streams with early customers. Consider creating a detailed financial projection model.'
        });
    }
    
    // Competitive Advantage recommendations
    const competitiveScore = scores.find(s => s.category === 'Competitive Advantage')?.score || 0;
    if (competitiveScore < 70) {
        recommendations.push({
            category: 'Competitive Strategy',
            recommendation: 'Identify and strengthen your unique value proposition. Analyze competitor strategies and find opportunities for differentiation.'
        });
    }
    
    // Team recommendations
    const teamScore = scores.find(s => s.category === 'Team Strength')?.score || 0;
    if (teamScore < 70) {
        recommendations.push({
            category: 'Team Building',
            recommendation: 'Consider adding team members with complementary skills, especially in areas like marketing, business development, or technical expertise.'
        });
    }
    
    // Execution recommendations
    const executionScore = scores.find(s => s.category === 'Execution Readiness')?.score || 0;
    if (executionScore < 70) {
        recommendations.push({
            category: 'Execution',
            recommendation: 'Develop a detailed go-to-market strategy and implementation roadmap. Consider starting with an MVP to test market response.'
        });
    }
    ...
    if (data.currentStage === 'idea') {
        recommendations.push({
            category: 'Next Steps',
            recommendation: 'Focus on validating your assumptions through customer interviews and creating a minimum viable product (MVP) to test market demand.'
        });
    }
...
function extractCategory(data) {
    ...
    if (text.includes('healthcare') || text.includes('medical') || text.includes('health')) return 'HealthTech';
    if (text.includes('education') || text.includes('learning') || text.includes('teach')) return 'EdTech';
    if (text.includes('finance') || text.includes('payment') || text.includes('bank')) return 'FinTech';
    if (text.includes('food') || text.includes('restaurant') || text.includes('delivery')) return 'FoodTech';
    if (text.includes('ai') || text.includes('machine learning') || text.includes('artificial intelligence')) return 'AI/ML';
    if (text.includes('environment') || text.includes('sustainable') || text.includes('green')) return 'ClimaTech';
    if (text.includes('ecommerce') || text.includes('e-commerce') || text.includes('retail')) return 'E-commerce';
    if (text.includes('social') || text.includes('community') || text.includes('network')) return 'Social';
    if (text.includes('mobile') || text.includes('app') || text.includes('software')) return 'Mobile/Software';
