...
const pool = new Pool({
  connectionString:
    ...
    "postgresql://username:password@localhost:5432/drishti_db",
  ssl:
...
const sampleLocalStorageData = {
  ...
  validationData: {
    ...
    problemStatement:
      "Small businesses struggle with customer service automation and need intelligent solutions that can handle complex inquiries while maintaining personal touch.",
    solutionDescription:
      "Our AI-powered platform provides intelligent customer service automation with natural language processing and machine learning capabilities.",
  ...
  userNotifications: [
    {
      ...
      message:
        "Jane Smith has expressed interest in your startup AI Assistant App with a $100,000 investment.",
      data: {
...
async function migrateStartups(validationData, validationResults, userData) {
  ...
  try {
    ...
    await client.query("ROLLBACK");
    console.error(
      `❌ Error migrating startup ${validationData.startupTitle}:`,
      error.message,
    );
    throw error;
  } finally {
...
function extractIndustry(data) {
  const text =
    `${data.problemStatement} ${data.solutionDescription} ${data.targetMarket}`.toLowerCase();
...
async function migrateData() {
  console.log(
    "🚀 Starting data migration from localStorage to PostgreSQL...\n",
  );
  ...
  try {
    ...
    console.log(
      `   • Notifications: ${sampleLocalStorageData.userNotifications.length} migrated`,
    );
  } catch (error) {
...
if (require.main === module) {
  ...
  switch (command) {
    ...
    default:
      ...
      console.log(
        "  node migrate_data.js export   - Export localStorage data to JSON",
      );
      console.log(
        "  node migrate_data.js backup   - Backup localStorage data to JSON",
      );
  }
}
